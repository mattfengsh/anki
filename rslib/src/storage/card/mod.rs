    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let clause = match self {
            ...
            ReviewOrderSubclause::RetrievabilityFsrs { timing, order } => {
                ...
                temp_string =
                    format!("if(extract_fsrs_variable(data, 's')<100, 0, 1), extract_fsrs_relative_retrievability(data, case when odue !=0 then odue else due end, ivl, {today}, {next_day_at}, {now})  {order},extract_fsrs_variable(data, 'd') asc, extract_fsrs_variable(data, 's') desc, due desc");
                &temp_string
            }
