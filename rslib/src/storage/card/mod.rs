    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let clause = match self {
            ...
            ReviewOrderSubclause::RetrievabilityFsrs { timing, order } => {
                ...
                temp_string =
                    format!("cast(round(extract_fsrs_relative_retrievability(data, case when odue !=0 then odue else due end, {today}, ivl, {next_day_at}, {now})*100) AS INTEGER) {order}, ord, round(extract_fsrs_variable(data, 's')) asc");
                &temp_string
            }
