    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let clause = match self {
            ...
            ReviewOrderSubclause::RetrievabilityFsrs { timing, order } => {
                ...
                temp_string =
                    format!("ord, floor(extract_fsrs_relative_retrievability(data, case when odue !=0 then odue else due end, {today}, ivl, {next_day_at})) {order}, floor(extract_fsrs_variable(data, 'd')) asc, due desc, ivl asc");
                &temp_string
            }
