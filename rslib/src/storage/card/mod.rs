    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let clause = match self {
            ...
            ReviewOrderSubclause::RetrievabilityFsrs { timing, order } => {
                ...
                temp_string =
                    format!("ord, cast(extract_fsrs_relative_retrievability(data, case when odue !=0 then odue else due end, {today}, ivl, {next_day_at})*100 as int) {order}, cast((extract_fsrs_variable(data, 'd')-1)/9*100 as int) asc, due desc, ivl asc");
                &temp_string
            }
